...
Begin VB.Form frmSignOn 
   ...
   Begin VB.Label Label3 
      ...
      Caption         =   "This is not the original code. Please see the about box for details of modifications made."
      Height          =   735
      Left            =   120
      TabIndex        =   9
      Top             =   2400
...
Dim iu$, hjg As String, l As Integer
...
Private Sub cmbScreenName_Change()
txtPassword.Text = ""

For X = 0 To List1.ListCount - 1
  If LCase$(List1.List(X)) = LCase$(cmbScreenName.Text) Then txtPassword.Text = List2.List(X)
  Next X
End Sub
...
If Text1.Text <> "" Then
    'I am in the process of implementing the MSN
    'protocol. Please ignore this code in the mean-time.
    
    found = False
    For X = 1 To Len(Text1.Text)
    If Mid$(Text1.Text, X, 1) = "@" Then found = True
    Next X
    If found = False Then MsgBox "You must enter a valid e-mail address into the MSN Messenger Passport E-mail field. YOU CAN LEAVE IT BLANK if you do not wish to use the MSN Messenger feature.", , "MSN problem.": Exit Sub
...
Private Sub Form_Load()
  ...
  For X = 0 To List1.ListCount - 1
  found = False
  For Y = 0 To cmbScreenName.ListCount - 1
  If LCase$(cmbScreenName.List(Y)) = LCase$(List1.List(X)) Then found = True
  Next Y
  If found = False Then cmbScreenName.AddItem List1.List(X)
  Next X
  
  If cmbScreenName.Text = "" Then cmbScreenName.Text = List1.List(0)
...
Winsock1.SendData "GET /" + Format$(Scode) + " HTTP 1.0" + vbCrLf + vbCrLf
...
If InStr(1, p, "404") Then
Winsock1.Close
Winsock1.RemoteHost = wskAIM.RemoteHostIP
  ...
  Select Case lngFrameType&
    ...
    Case 2 'a frame type of "2" indicates normal data
      ...
      Case "GOTO_URL"
          ...
          InfoText = GetURLSource2(arrArgs$(1))
          
          a = InStr(1, InfoText, "Warning Level : <B>")
          Temp = Right$(InfoText, Len(InfoText) - (a + 18))
          b = InStr(1, Temp, "%")
          warning = Left$(Temp, b)
          frmInfo.WarnLevel.Caption = warning
          
          a = InStr(1, InfoText, "Online Since : <B>")
          Temp = Right$(InfoText, Len(InfoText) - (a + 17))
          b = InStr(1, Temp, vbLf)
          olinetime = Left$(Temp, b - 1)
          frmInfo.OnTime.Caption = olinetime
          
          'Idle Minutes :
          
          a = InStr(1, InfoText, "Idle Minutes : <B>")
          Temp = Right$(InfoText, Len(InfoText) - (a + 17))
          b = InStr(1, Temp, "<")
          olinetime = Left$(Temp, b - 1)
          If olinetime = "0" Then frmInfo.UsrStatus.Caption = "Online" Else frmInfo.UsrStatus.Caption = "Idle"
          
          c = a + b + 17
          Temp = Right$(InfoText, Len(InfoText) - c)
          d = InStr(1, Temp, "<hr>")
          temp2 = Right$(Temp, Len(Temp) - 17)
          e = InStr(1, temp2, "Legend:")
          profile = Left$(temp2, (e - 12))
          InfoText = profile
...
'-=-
      ...
      End If
      arse$ = ""
      Call SendProc(2, "toc_set_caps 0x003 0x004 0x005 0x006 0x007" & Chr$(0))
...
'        Call SendProc(2, "toc_set_config " & BuddyConfig$ & Chr(0))
        ...
        '
        If sentsomething = False Then
        Call SendProc(2, "toc_set_config " & BuddyConfig$ & Chr(0))
        Call SendProc(2, "toc_add_permit " & m_strPDList$ & Chr(0))
        End If
        Call SendProc(2, "toc_init_done" & Chr(0))
        
        iu$ = "<HTML><BODY BGCOLOR=" & Chr(34) & "#ffffff" & Chr(34) & "><FONT COLOR=" & Chr(34) & "#00ff00" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>[<U></FONT><FONT COLOR=" & Chr(34) & "#ff8000" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>Personal</U></FONT><FONT COLOR=" & Chr(34) & "#ff8000" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1> <U></FONT><FONT COLOR=" & Chr(34) & "#ff8000" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>Profile</U></FONT><FONT COLOR=" & Chr(34) & "#00ff00" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>]: </FONT></BODY></HTML><HTML><font color=#000000>I'm using a VBaim example by <B>Chad Cox</B>, <B>Thomas Grimshaw</B> and <B>Steve Nowiki</B>, with Ragno Web Products.</font></HTML>"
  
        For lngTreeLoop& = 1 To frmBuddyList.tvwBuddies.Nodes.Count
        frmBuddyList.tvwBuddies.Nodes.Item(lngTreeLoop&).Sorted = True
        Next lngTreeLoop&
        For lngTreeLoop& = 1 To frmBuddyList.tvwSetup.Nodes.Count
        frmBuddyList.tvwSetup.Nodes.Item(lngTreeLoop&).Sorted = True
        Next lngTreeLoop&
        hjg = bGetRegValue("Software\vbAIM Example", "Profile")
        If hjg = "" Then
        l = CreateKey("HKEY_LOCAL_MACHINE\Software\vbAIM Example")
        
        l = bSetRegValue(HKEY_LOCAL_MACHINE, "Software\vbAIM Example", "Profile", "2")
        l = bSetRegValue(HKEY_LOCAL_MACHINE, "Software\vbAIM Example", "Profile1", Left$(iu$, 250))
        l = bSetRegValue(HKEY_LOCAL_MACHINE, "Software\vbAIM Example", "Profile2", Right$(iu$, Len(iu$) - 250))
        Else
        iu$ = ""
        For X = 1 To Val(hjg)
        ...
        l = bSetRegValue(HKEY_LOCAL_MACHINE, "Software\vbAIM Example", "Profile1", Left$(iu$, 250))
        l = bSetRegValue(HKEY_LOCAL_MACHINE, "Software\vbAIM Example", "Profile2", Right$(iu$, Len(iu$) - 250))
        iu$ = Normalize("<HTML><BODY BGCOLOR=" & Chr(34) & "#ffffff" & Chr(34) & "><FONT COLOR=" & Chr(34) & "#00ff00" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>[<U></FONT><FONT COLOR=" & Chr(34) & "#ff8000" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>Personal</U></FONT><FONT COLOR=" & Chr(34) & "#ff8000" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1> <U></FONT><FONT COLOR=" & Chr(34) & "#ff8000" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>Profile</U></FONT><FONT COLOR=" & Chr(34) & "#00ff00" & Chr(34) & " FACE=" & Chr(34) & "Verdana" & Chr(34) & " SIZE=1>]: </FONT></BODY></HTML><HTML>I'm using a VBaim example by Chad Cox, Thomas Grimshaw and Steve Nowiki, with Ragno Web Products.</HTML>")
        End If
        End If
        
        Call SendProc(2, "toc_set_info " & Chr(34) & Normalize(iu$) & Chr(34) & Chr(0))
        Timer1.Enabled = True
        Case "CHAT_IN"
          ...
          If lngFormIndex& > -1 Then
            ...
            'here we update in red if the message is our's and blue if not.
              Forms(lngFormIndex&).List1.AddItem arrArgs$(1) & ": " & KillHTML(arrArgs$(3))
              Forms(lngFormIndex&).List1.Selected(Forms(lngFormIndex&).List1.ListCount - 1) = True
            Else
              Forms(lngFormIndex&).List1.AddItem arrArgs$(1) & ": " & KillHTML(arrArgs$(3))
              Forms(lngFormIndex&).List1.Selected(Forms(lngFormIndex&).List1.ListCount - 1) = True
            End If
        ...
        Case "CHAT_JOIN"
          ...
          If lngFormIndex& < 0 Then
            ...
            With frmNewChat
              ...
              .Show
            End With
            frmNewChat.List1.AddItem "SYSTEM: *** You have joined " & Chr(34) & arrArgs$(1) & Chr(34)
          Else
          Forms(lngFormIndex&).rtfDisplay.List1.AddItem "SYSTEM: *** You have joined " & Chr(34) & arrArgs$(1) & Chr(34)
            ...
            If arrArgs$(1) = "T" Then
              ...
              For lngNameLoop& = LBound(arrNames$) To UBound(arrNames$)
                ...
                If blnShowJoin = True Then
                  Forms(lngFormIndex&).List1.AddItem "SYSTEM: *** " & arrNames$(lngNameLoop&) & " has joined " & Forms(lngFormIndex&).Caption
                End If
              Next
            Else
              For lngNameLoop& = LBound(arrNames$) To UBound(arrNames$)
                For lngListLoop& = 0 To Forms(lngFormIndex&).lstNames.ListCount - 1
                  If Forms(lngFormIndex&).lstNames.List(lngListLoop&) = arrNames$(lngNameLoop&) Then
                    ...
                    Forms(lngFormIndex&).lblPeople.Caption = Forms(lngFormIndex&).lstNames.ListCount & " people here"
                    Forms(lngFormIndex&).List1.AddItem "SYSTEM: *** " & arrNames$(lngNameLoop&) & " has left " & Forms(lngFormIndex&).Caption
                    Exit For
                  End If
                Next
              Next
        ...
        Case "IM_IN"
          ...
          If lngFormIndex& > -1 Then
            Call RTFUpdate(Forms(lngFormIndex&).rtfDisplay, htmltorich2("<font color=""#FF0000"" size=3>" & arrArgs(0) & ": </font><font color=""#000000"" face=""Times New Roman"">" & arrArgs$(2)))
          ...
          Else
            ...
            End With
            Call RTFUpdate(frmNewIM.rtfDisplay, htmltorich2("<font color=""#FF0000"" size=3>" & arrArgs(0) & ": </font><font color=""#000000"" face=""Times New Roman"">" & arrArgs$(2)))
          ...
          'frmAway.List1.AddItem arrArgs$(0)
            ...
            ph$ = awaymessage$
                ...
                Call SendProc(2, "toc_send_im " & Replace(arrArgs$(0), " ", "") & " " & Chr(34) & Normalize("<HTML>" & ph$ & "</HTML>") & Chr(34) & " " & "auto" & Chr(0))
                Call RTFUpdate(Forms(lngFormIndex&).rtfDisplay, htmltorich2("<font color=""#0000FF"" size=3>" + m_strFormattedSN$ + " </font><font color=""#000000"" size=3 face=""times new roman""></p>" + ph$))
